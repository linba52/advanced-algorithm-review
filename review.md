...
一个小例子可以帮助理解。假设 Alice、Bob、Carol 都在同一个长度为 `n` 的数组里各自
申请一个槽位。普通指针中，`p = 5` 一般表示固定的第 5 个槽位。但 tiny pointer 中，
`p = 5` 可以有三种不同解释：

```text
Dereference(Alice, 5) = slot 17
Dereference(Bob,   5) = slot 804
Dereference(Carol, 5) = slot 42
```

这不是编码冲突，因为 tiny pointer 的语义从来不是“5 号全局地址”，而是“该 owner 的
第 5 个局部选择”。因此 tiny pointer 压缩的不是物理地址本身，而是 owner 上下文之外
还缺少的那部分信息。
...
截至本 review 写作时，我没有找到明显以后续论文标题或摘要直接围绕“tiny pointers”
展开的工作。更准确地说，
...
为了让没有读过原论文的同学能够检查本文解释是否可信，可以按下面的路线复核。

第一步，检查模型是否真的避开了普通指针下界。普通下界说：若 `p` 单独指向 `n` 个槽位
之一，则需要 `log n` 位。Tiny pointer 模型中，槽位由 `(k, p)` 和表的随机元数据共同
确定，所以原下界的前提不成立。只要确认 `Dereference` 的输入确实包含 `k`，就能理解
为什么问题变了。

第二步，检查固定长度上界。需要验证四个点：

- 主表 bucket 内编号只要 `O(log delta^{-1})` 位；
- 主表失败的活跃分配数量高概率很少；
- 副表在稀疏负载下只要 `O(log log log n)` 位局部编号；
- 主表和副表加起来总 slot 数仍是 `n` 量级。

如果这四点都成立，则固定长度上界
`O(log log log n + log delta^{-1})` 基本成立。

第三步，检查可变长度上界。关键是两个概率和：

```text
sum_i Pr[普通第 i 层成功] * O(log i)
sum_i Pr[第 i 层 overflow] * O(log s_i)
```

普通层的概率按几何级数下降，overflow 概率按 `exp(-Omega(s_i))` 下降。两个和都是
常数，因此常数负载下期望 pointer 长度是 `O(1)`。再通过主表/副表归约得到一般负载下
的 `O(1 + log delta^{-1})`。

第四步，检查固定长度下界。若固定 pointer 只有 `s` 位，则每个 key 只有 `2^s` 个候选。
把 dereference table 转成 sequential balls-and-bins 过程后，经典下界要求
`2^s = Omega(log log n)`，因此 `s = Omega(log log log n)`。这个复核点最重要，因为它
说明 `log log log n` 不是构造懒惰造成的，而是固定长度模型中不可避免。

第五步，检查可变长度下界。高负载下空槽很少，短候选集合不可能对随机插入都足够好；
potential 论证说明常数比例的插入必须用超过 `Theta(delta^{-1})` 的候选编号，因此期望
需要 `Omega(log delta^{-1})` 位。

如果这五步都能跟下来，就基本能验证论文的主定理：固定长度和可变长度 tiny pointers 的
空间界都是紧的。
